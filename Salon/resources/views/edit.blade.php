...
<div class="col-lg-10 col-12 badge-dark" id="box">
  <h1> BOOK YOUR APPOINTMENT</h1>
    ...
    @csrf Name :<input type="text"name="name" class="text-dark" value="{{ $product->name }}" required><br>
    @error('name')
    <div style="color:red">{{$message}}</div>
  @enderror
    Email :<input type="text"  name="email"  class="text-dark" value="{{ $product->email }}" required><br>
    @error('email')
    <div style="color:red">{{$message}}</div>
  @enderror
    ...
    Mobile Number :<input type="text"  value="{{ $product->mobile }}" class="text-dark" name="mobile" required><br>
    @error('mobile')
    <div style="color:red">{{$message}}</div>
  @enderror<br>
    Select Course:
